...
class EventBasedSamplingDataset(SamplingDataset):
    def __getitem__(self, index: int) -> Tensor:
        ...
        for i in range(num_trials):
            ...
            event_id = torch.cat([index, event_id])  
            event_ids.append(event_id)
        return event_ids
...
def make_split_loaders(data: TemporalData, config):
    """ 
    Builds train/validation/test dataloaders under a transductive,
    chronological split: interaction events are ordered by timestamp
    and partitioned into three consecutive, non-overlapping subsets
    (train, validation, test), preserving temporal ordering to prevent 
